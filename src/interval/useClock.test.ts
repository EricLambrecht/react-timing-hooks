...
describe('useClock', () => {
  ...
  it('displays/returns the correct time, every second', () => {
    ...
    expect(result.current).toBe('1:59:14 PM')

    act(() => {
      jest.advanceTimersByTime(1000)
    })
    expect(result.current).toBe('1:59:15 PM')

    act(() => {
      jest.advanceTimersByTime(1000)
    })
    expect(result.current).toBe('1:59:16 PM')

    act(() => {
      jest.advanceTimersByTime(2000) // 2(!) seconds
    })
    expect(result.current).toBe('1:59:18 PM')
  })

  it('supports custom formatting', () => {
    const testTimeInMilliseconds = 1642251554998 // Jan 15th 2022, 1:59:14 PM
    const testFormatter = (date: Date) => date.toLocaleTimeString('de-DE')
